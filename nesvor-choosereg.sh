...
best="$1"
fullbest=`readlink -f ${best}`
dir=`dirname $fullbest`
if [[ ! -f $best ]] ; then
...
if [[ ${base} == *"FLIRT"* ]] ; then 
    ...
    echo "not a flirt transform, we assume you want to finalize the input image"
fi

recondir=`dirname $dir`
id=`basename $recondir`

subj=`echo $id | sed -e 's,s.*,,g'`
scan=`echo $id | sed -e 's,.*\(s.*\),\1,g'`

anat=${dir}/BIDS/sub-${subj}/${scan}/anat
mkdir -pv ${anat}

cp $best -v ${anat}/sub-${subj}_${scan}_rec-nesvor_t2w.nii.gz
