...
echo "Deoblique..."
cmd="/opt/x86_64/pkgs/afni_latest/3dWarp -deoblique -prefix $drecon $recon"
echo "$cmd" >> $script
$cmd
echo
...
if [[ -n $2 ]] ; then
    ...
    for file in ${REFDIR}/fetus*.nii* ; do
        echo $file
        base=`basename $file`
        cmd="/fileserver/fetal/bin/crlReorientReconstructedImage $drecon $file ${dir2}/r3DreconO$base"
        echo "$cmd" >> $script
        $cmd
        echo
    done
