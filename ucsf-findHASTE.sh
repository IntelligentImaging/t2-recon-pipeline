...
for acq in ${nii}/* ; do
    if [[ -d $acq ]] ; then
        ...
        # Take dimension 1 (in plane) and 3 (out-of-plane) spacings
        ...
        # T2's normally have have in plane thickness less than 1
        # though I noticed some in this dataset are over 1(?), so I'm excluding 2 or greater
        # For out of plane, I'm excluding those with thickness 4 or greater
        if [[ $ddim1 -gt 1 || $ddim3 -gt 3 ]] ; then
            ...
            base=`basename $im`
            fetus="fetus${base}"
            # Copy the selected images to the reconstruction directory
            cp $im -vn ${odir}/${fetus}
        fi
    fi
done
